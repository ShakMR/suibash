...
class Distance():
    ...
    def needelman(str1, str2, G=2):
        ...
        for i in range(1, len(str1)+1):
            for j in range(1, len(str2)+1):
                d[i][j] = min(d[i][j-1]+1+G, d[i-1][j]+1+G, d[i-1][j-1]+(not str1[i-1] == str2[j-1]))
        return d[len(str1)][len(str2)]
    ...
    def get_similarities(str1, lst, sim_func, max_dist=100, with_ham=True):
        distancies = map(lambda x: (x, Distance.distance(str1, x, sim_func, with_ham)), lst)
        distancies.sort(key=lambda x: (x[1], x[0]))
        mindist = min(distancies[0][1], max_dist)
        list = map(lambda x: x[0], filter(lambda x: x[1] <= mindist, distancies))
        try:
